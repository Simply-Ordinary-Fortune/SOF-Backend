...
let storedCode = null; // 프론트에서 받은 OAuth 인증 코드 저장
...
// 1. 프론트에서 OAuth 인증 후 받은 code를 저장하는 엔드포인트
export const saveAuthCode = (req, res) => {
  const { code } = req.body;
  if (!code) return res.status(400).json({ error: "Authorization code is missing" });

  storedCode = code;
  console.log("Received and stored auth code:", storedCode);
  res.json({ message: "Authorization code received and stored" });
};

// 2. Google OAuth 콜백 처리 (토큰 요청)
export const handleOAuthCallback = async (req, res) => {
  try {
    if (!storedCode) return res.status(400).json({ error: "No stored authorization code found" });

    // 저장된 인증 코드로 Access Token 요청
    const { tokens } = await oAuth2Client.getToken(storedCode);
    oAuth2Client.setCredentials(tokens);
